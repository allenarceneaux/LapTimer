...
function lib.newGUI()
  ...
  function gui.drawTriangle(x1, y1, x2, y2, x3, y3, flags)
    ...
    lcd.drawLine(x2, y2, x3, y3, SOLID, flags)
    lcd.drawLine(x3, y3, x1, y1, SOLID, flags)
  end
  ...
  local function drawFocus(x, y, w, h, color)
    ...
    gui.drawRectangle(x - 2, y + 4, w + 4, h - 1, color, 2)
...
-- Create a text label
  function gui.label(x, y, w, h, title, flags)
    local self = {
      title = title,
      flags = bit32.bor(flags or lib.flags, LEFT, lib.colors.primary1),
      disabled = true
    }
...
-- Create a toggle button that turns on/off. callBack gets true/false
  function gui.toggleButton(x, y, w, h, title, value, callBack, flags)
    local self = {
      ...
      value = value,
      callBack = callBack or doNothing,
      flags = bit32.bor(flags or lib.flags, LEFT)
    }
  ...
  function gui.menu(x, y, w, h, items, callBack, flags)
    ...
    function self.draw(focused)
      ...
      for i = 0, visibleCount - 1 do
        ...
        if j == selected then
          gui.drawFilledRectangle(x-1, y + (lh / 2) - 1, w, lh - 3, bgColor)
  ...
  function gui.dropDown(x, y, w, h, items, selected, callBack, flags)
    ...
    function self.draw(focused)
      if drawingMenu then
        ...
        local flags = bit32.bor(LEFT, lib.colors.primary1, getFlags(self))
